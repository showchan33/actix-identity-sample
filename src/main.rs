use actix_web::{
  web, get, post, App, HttpServer, HttpResponse, HttpMessage,
  Responder, HttpRequest, cookie::Key};
...
async fn login(
  ...
  if form.username == "user" && form.password == "password" {
    ...
    HttpResponse::Found().append_header(("location", "/welcome")).finish()
  } else {
...
async fn secret() -> HttpResponse {
  HttpResponse::Ok().body(
    format!("This page is only accessible to authenticated users.")
  )
}
